 ...
 */
;(function ($) {
  function createCode(dom, config, advicewidth) {
    var _this = this;
    _this.config = config
    ...
    for (i = 0; i < _this.codeLen; i++) {
      ...
      var w = _this.codeLen * 20;
      if (advicewidth != undefined && !isNaN(advicewidth)) {
          w = advicewidth;
      }
    $(dom).css({
      width: w + 'px',
  ...
  $.fn.createCode = function (cfg,advicewidth) {
    var _this = this;
      createCode(_this, cfg, advicewidth);
